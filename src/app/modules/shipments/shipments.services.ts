  ...
  const shipment = await prisma.shipments.findUnique({
    ...
    select: {
      ...
      shipmentHistory: {
        ...
        select: { status: true, remarks: true, updatedAt: true },
      },
    },
  });
...
const updateShipmentStatus = async (
  ...
  const isInterDivision =
    shipment.merchant.division !== shipment.receiverDivision;
  const transitions = isInterDivision
    ? INTER_DIVISION_TRANSITIONS
    : SAME_DIVISION_TRANSITIONS;
  ...
  if (!allowed || !allowed.includes(newStatus))
    throw new AppError(
      httpStatus.BAD_REQUEST,
      isInterDivision &&
        currentStatus === ShipmentStatus.PAID &&
        newStatus === ShipmentStatus.READY_FOR_ASSIGNMENT
